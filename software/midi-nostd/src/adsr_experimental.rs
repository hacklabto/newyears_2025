    ...
    > CoreAdsr<PLAY_FREQUENCY, A, D, SUSTAIN_VOLUME, R>
{
    ...
    const A_GAIN : AdsrFraction = if Self::A_TICKS != 0 {
        ...
        AdsrFraction::new(
            (a_diff/(A as i64)) as i32,
            ((a_diff)%(A as i64) * ADSR_FRACTION_DENOMINATOR / (A as i64)) as i32
        )
    }
    else {
    ...
    const D_GAIN : AdsrFraction = if Self::D_TICKS != 0 {
        ...
        AdsrFraction::new(
            (d_diff/(D as i64)) as i32,
            ((d_diff)%(D as i64) * ADSR_FRACTION_DENOMINATOR / (D as i64)) as i32
        )
    }
    else {
        AdsrFraction::new(0,0)
